							...
							struct serial_configure *cfg)
{
    ...
    UART_IntConfig(UART_0, UART_INTCFG_THRE, ENABLE);
...
static int put_char(struct rt_serial_device *serial, char c)
{
	...
	{
		LPC_UART0->THR = c & UART_THR_MASKBIT;
		SendFifoLength ++;
	}
...
static inline uint32_t writeSendFifo(void)
{
	...
	for( ; SendFifoLength < 16; SendFifoLength ++ )
	{
		if( SendLength > 0)
		{
			LPC_UART0->THR = (*SendPtr) & UART_THR_MASKBIT;
			SendPtr ++;
			SendLength --;
			write_cnt ++;
		}
...
void rt_hw_usart0_init(void)
{
    ...
    rt_hw_serial_register(&serial_device, "serial0",
...
void UART0_IRQHandler(void)
{
    ...
    intsrc = (LPC_UART0->IIR) & UART_IIR_BITMASK;
    ...
    {
        // Check line status
        lsr = (LPC_UART0->LSR) & UART_LSR_BITMASK;
